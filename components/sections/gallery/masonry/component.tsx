...
import type { SiteContent, GalleryImage } from '@/lib/content';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Animate variant="fade-up">
          <div className="mb-14 max-w-xl">
            {gallery.label && (
              <p className="text-xs font-bold tracking-[0.2em] uppercase mb-3" style={{ color: 'var(--color-accent)' }}>
                {gallery.label}
              </p>
            )}
            <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-white" style={{ fontFamily: 'var(--font-barlow, system-ui)' }}>
              {gallery.heading || 'Our Work'}
            </h2>
            {gallery.subheading && (
              <p className="mt-3 text-white/50 text-base leading-relaxed">{gallery.subheading}</p>
            )}
          </div>
        </Animate>
